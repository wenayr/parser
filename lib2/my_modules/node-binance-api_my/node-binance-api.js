...
let api = function Binance( options = {} ) {
    ...
    const reqHandler = cb => ( error, response, body ) => {
        ...
        if ( response ) {
            ...
            if ( response.headers ) {
                Binance.info.usedWeight = response.headers['x-mbx-used-weight-1m'] ?? response.headers['x-sapi-used-ip-weight-1m'] ?? 0;
                Binance.info.usedIPWeight1m = Binance.info.usedWeight;
                Binance.info.usedIPWeight1s = response.headers['x-mbx-used-weight-1s'] ?? response.headers['x-sapi-used-ip-weight-1s'];
                Binance.info.usedUIDWeight1m = response.headers['x-sapi-used-uid-weight-1m'];
                Binance.info.usedUIDWeight1s = response.headers['x-sapi-used-uid-weight-1s'];
                ...
                Binance.info.headers = response.headers;
            }
        }
        if ( !cb ) return;
    ...
    const promiseRequest = async ( url, data = {}, flags = {} ) => {
        return new Promise( ( resolve, reject ) => {
            ...
            if ( flags.type === 'SIGNED' || flags.type === 'TRADE' || flags.type === 'USER_DATA' || (Binance.options.APISECRET && baseURL!==base && baseURL!==sapi)) {
            ...
            //if (flags.type==="MARKET_DATA") data.timestamp = new Date().getTime() + Binance.info.timeOffset;
            opt.qs = data;
     ...
     */
        ...
        websockets: {
           ...
           */
             ...
             * @param {string[]} urls - the websocket endpoints
             * @param {function} callback - optional execution callback
             * @param {boolean} reconnect - subscription callback
             * @return {WebSocket} the websocket reference
             */
            subscribeCombined: function ( urls, callback, reconnect = false ) {
                return subscribeCombined( urls, callback, reconnect );
            },
